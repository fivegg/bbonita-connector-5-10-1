 ...
 */
public class CreateContactIntoExchangeServerTest extends TestCase {
	...
	private String email1   = "jordi@gmail.com";
	private String email2   = "jordi@hotmail.com";
	
	protected static final Logger LOG = Logger.getLogger(CreateContactIntoExchangeServerTest.class.getName());
	...
	public void testMinimumParameters() throws BonitaException {
		CreateCalendarItem connector = getValidConnector();
	...
	public void testWrongAttendeeEmail() throws BonitaException {
		CreateCalendarItem connector = getValidConnector();
		connector.setRequiredAttendeesEmails("abcdfg");
		try {
			List<ConnectorError> errors =  connector.validateValues();
			assertTrue( errors.size() == 0 );
			
			connector.executeConnector();
			assertTrue( connector.getStatusCode().equals("ErrorInvalidRecipients") );
			assertTrue( connector.getStatusText().equals("Error") );
			assertTrue( connector.getResponse().length() == 0 );
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);
	...
	public void testOneAttendeeEmail() throws BonitaException {
		CreateCalendarItem connector = getValidConnector();
		connector.setRequiredAttendeesEmails(email1);
		try {
			List<ConnectorError> errors =  connector.validateValues();
			assertTrue( errors.size() == 0 );
	...
	public void testFourAttendeeEmail() throws BonitaException {
		CreateCalendarItem connector = getValidConnector();
		connector.setRequiredAttendeesEmails(email1 + "," + email2 + "," + email1 + "," + email2 );
		try {
			List<ConnectorError> errors =  connector.validateValues();
			assertTrue( errors.size() == 0 );
	...
	private CreateCalendarItem getValidConnector() {
		...
		String subject   = "Saint George book's day";
		String body      = "Meeting : book selling strategy during Saint George's day<br>www.bonitasoft.com";
		String startTime = "2010-04-03T09:00:00";
		String endTime   = "2010-04-03T11:00:00";
		String location  = "Conference room 007";
		String requiredAttendeesEmails = email1 + ", " + email2;
