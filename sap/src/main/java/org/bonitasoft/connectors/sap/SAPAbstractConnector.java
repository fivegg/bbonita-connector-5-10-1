  ...
  protected List<ConnectorError> validateValues() {
    ...
    if (useExitingConnection) {
      ...
      if (isNull(serverType)) {
        errors.add(new ConnectorError("serverType", new IllegalArgumentException("Server type MUST BE DEFINED. " +
            "It MUST BE one of: " + SERVER_TYPE_APPLICATION_SERVER + ", " + SERVER_TYPE_MESSAGE_SERVER)));
      }
      if (isNull(client)) {
        errors.add(new ConnectorError("client", new IllegalArgumentException("Client MUST BE DEFINED. ")));
      }
      if (isNull(user)) {
        errors.add(new ConnectorError("user", new IllegalArgumentException("User MUST BE DEFINED. ")));
      }
      if (isNull(password)) {
        errors.add(new ConnectorError("password", new IllegalArgumentException("Password MUST BE DEFINED. ")));
      }
      if (isNull(language)) {
        errors.add(new ConnectorError("language", new IllegalArgumentException("Language MUST BE DEFINED. ")));
      }
      if (isNull(host)) {
        errors.add(new ConnectorError("host", new IllegalArgumentException("Host MUST BE DEFINED. ")));
      }
      if (SERVER_TYPE_APPLICATION_SERVER.equals(serverType) && isNull(systemNumber)) {
        errors.add(new ConnectorError("systemNumber", new IllegalArgumentException("If server type is " + SERVER_TYPE_APPLICATION_SERVER + ", systemNumber MUST BE DEFINED")));
      }
      /*if (SERVER_TYPE_APPLICATION_SERVER.equals(serverType) && isNotNull(systemId)) {
        errors.add(new ConnectorError("systemId", new IllegalArgumentException("If server type is " + SERVER_TYPE_APPLICATION_SERVER + ", systemId MUST BE NULL")));
      }
      if (SERVER_TYPE_APPLICATION_SERVER.equals(serverType) && isNotNull(groupName)) {
        errors.add(new ConnectorError("groupName", new IllegalArgumentException("If server type is " + SERVER_TYPE_APPLICATION_SERVER + ", groupName MUST BE NULL")));
      }*/

      if (SERVER_TYPE_MESSAGE_SERVER.equals(serverType) && isNull(systemId)) {
        errors.add(new ConnectorError("systemId", new IllegalArgumentException("If server type is " + SERVER_TYPE_MESSAGE_SERVER + ", systemId MUST BE DEFINED")));
      }
      if (SERVER_TYPE_MESSAGE_SERVER.equals(serverType) && isNull(groupName)) {
        errors.add(new ConnectorError("groupName", new IllegalArgumentException("If server type is " + SERVER_TYPE_MESSAGE_SERVER + ", groupName MUST BE DEFINED")));
      }
      
      /*if (SERVER_TYPE_MESSAGE_SERVER.equals(serverType) && isNotNull(systemNumber)) {
        errors.add(new ConnectorError("systemNumber", new IllegalArgumentException("If server type is " + SERVER_TYPE_MESSAGE_SERVER + ", systemNumber MUST BE NULL")));
      }*/
    }
    return errors;
  }
