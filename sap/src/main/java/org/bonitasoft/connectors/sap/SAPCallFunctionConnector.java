		...
		if (inputParameters != null) {
			...
			for (List<Object> row : inputParameters) {
				if (row.size() != EXPECTED_INPUT_COLUMN_NUMBER) {
					errors.add(new ConnectorError("inputParameters",
							new IllegalArgumentException("Row number: "
									+ inputRowNb
									+ " has a wrong number of columns: "
									+ row.size() + " (instead of "
									+ EXPECTED_INPUT_COLUMN_NUMBER + ").")));
				...
				if (!INPUT_PARAMETER_TYPES.contains(row.get(0))) {
					errors.add(new ConnectorError("inputParameters",
							new IllegalArgumentException("Row number: "
									+ inputRowNb
									+ " has a wrong value for parameterType: "
									+ row.get(0) + " (instead of "
		...
		if (outputParameters != null) {
			...
			for (List<String> row : outputParameters) {
				if (row.size() != EXPECTED_OUTPUT_COLUMN_NUMBER) {
					errors.add(new ConnectorError("outputParameters",
							new IllegalArgumentException("Row number: "
									+ outputRowNb
									+ " has a wrong number of columns: "
									+ row.size() + " (instead of "
									+ EXPECTED_OUTPUT_COLUMN_NUMBER + ").")));
